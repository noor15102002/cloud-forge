...
import subprocess
import tempfile
from core_fixture import prepare, require_core_healthy
...
parser.add_argument("--core", action="store_true", help="Use the explicit HTTP-core profile; experimental cases stay in the original suite")
args = parser.parse_args()
with tempfile.TemporaryDirectory(prefix="cf-core-fixture-") as profile_root:
    ...
    for number in range(1, 6):
        ...
        result = run_observed([args.binary, "verify", str(application), "--format", "json"], path, timeout=900)
        ...
        required = ["container-build", "deployment-readiness", "pod-recovery", "rolling-deployment", "load-profile"]
        if not args.core:
            required += ["readiness-gating", "inflight-shutdown"]
        else:
            require_core_healthy(report, result.returncode)
