...
func TestLoadRejectsSchemaMismatchUnknownFieldsAndDuplicateMetrics(t *testing.T) {
	...
	for name, document := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if name == "schema" && !strings.Contains(err.Error(), `expected v1alpha1, v1alpha2, v1alpha3, v1alpha4, v1alpha5, v1alpha6, v1alpha7 or "v1alpha8"`) {
...
func TestBuildSchemaMatchesPublishedContract(t *testing.T) {
	published, err := os.ReadFile(filepath.Join("..", "..", "schemas", "verification.v1alpha4.schema.json"))
	if err != nil {
		t.Fatal(err)
	}
